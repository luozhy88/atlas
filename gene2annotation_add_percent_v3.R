...
eggNog_df <- read.delim2("Genecatalog/annotations/eggNog.tsv.gz",sep = "\t",header=2)[c(-1),]
...
Nmapped_count=sweep(Nmapped_count,2,colSums(Nmapped_count),"/") %>% data.frame()
...
KEGG_Module_c[,-1]<-apply(KEGG_Module_c[,-1], 2, as.numeric) %>% data.frame()
...
KO_c[,-1]<-apply(KO_c[,-1], 2, as.numeric) %>% data.frame()
...
ca_c[,-1]<-apply(ca_c[,-1], 2, as.numeric) %>% data.frame()
...
EC_c[,-1]<-apply(EC_c[,-1], 2, as.numeric) %>% data.frame()
...
KEGG_Pathway_c[,-1]<-apply(KEGG_Pathway_c[,-1], 2, as.numeric) %>% data.frame()
...
PFAMs_c[,-1]<-apply(PFAMs_c[,-1], 2, as.numeric) %>% data.frame()
